...
<%
    ...
    try {
        ...
        if (feedResponse.getStatus() == FeedResponse.SUCCESS) {
            ...
            accounts = new UserUploadService().getSurveyGizmoSurveys(dataSource);
        } else {
            throw new RuntimeException();
        }
        if (accounts.size() == 1) {
            response.sendRedirect("/app/html/dataSources/" + request.getParameter("dataSourceID") + "/surveyGizmoSurveyChoice/" + accounts.iterator().next().getId() + "/" + URLEncoder.encode(accounts.iterator().next().getName(), "UTF-8"));
            return;
        }
%>
...
<div class="container corePageWell">
    <div class="row">
        <div class="col-md-6 col-md-offset-3">
            <div>We found the following options as available Survey Gizmo surveys:</div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-6 col-md-offset-3">
            ...
            <%
                for (BasecampNextAccount account : accounts) {
                    %>
                <li>><a href="/app/html/dataSources/<%=request.getParameter("dataSourceID")%>/surveyGizmoSurveyChoice/<%=account.getId()%>/<%=URLEncoder.encode(account.getName(), "UTF-8")%>"><%= account.getName()%></a></li>
                    <%
                }
            %>
            </ul>
