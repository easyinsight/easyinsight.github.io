...
<%
    ...
    try {
        ...
        if (feedResponse.getStatus() == FeedResponse.SUCCESS) {
            ...
            accounts = new UserUploadService().getSmartsheetTables(dataSource);
        } else {
            throw new RuntimeException();
        }
        if (accounts.size() == 1) {
            BasecampNextAccount account = accounts.iterator().next();
            response.sendRedirect("/app/html/dataSources/" + request.getParameter("dataSourceID") + "/smartsheetAccountChoice/" + account.getId() + "/" + account.getName());
            return;
        }
%>
...
<div class="container corePageWell">
    <div class="row">
        <div class="col-md-6 col-md-offset-3">
            <div>We found the following options as Smartsheet tables:</div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-6 col-md-offset-3">
            ...
            <%
                for (BasecampNextAccount account : accounts) {
                    %>
                <li>><a href="/app/html/dataSources/<%=request.getParameter("dataSourceID")%>/smartsheetAccountChoice/<%=account.getId()%>/<%=URLEncoder.encode(account.getName(), "UTF-8")%>"><%= account.getName()%></a></li>
                    <%
                }
            %>
            </ul>
