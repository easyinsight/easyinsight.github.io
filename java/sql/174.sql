...
create table email_selenium_processor (
  ...
  constraint email_selenium_processor_ibfk2 foreign key (scheduled_account_activity_id) references scheduled_account_activity (scheduled_account_activity_id) on delete cascade
);
...
create table selenium_request (
  ...
  constraint selenium_request_ibfk3 foreign key (account_id)  references account (account_id) on delete cascade
);