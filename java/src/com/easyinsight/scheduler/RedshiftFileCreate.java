 ...
 * Time: 9:10:02 PM
 */
public class RedshiftFileCreate {
    ...
    public void createFeed(Connection conn, byte[] bytes, UploadFormat uploadFormat, List<AnalysisItem> fields, boolean accountVisible) throws Exception {
        ...
        try {
            ...
            String tempTableName = tempStorage.getTableName();
            ...
            for (CharsetMatch charsetMatch : charsetMatches) {
                try {
                    ...
                    // continue
                    throw new ReportException(new DataSourceConnectivityReportFault("Could not match to a character set encoding.", feedDefinition));
                }
            }
            if (r == null) {
                throw new ReportException(new DataSourceConnectivityReportFault("Could not match to a character set encoding.", feedDefinition));
            ...
            tableDef.insertFromSelect(tempTableName);
