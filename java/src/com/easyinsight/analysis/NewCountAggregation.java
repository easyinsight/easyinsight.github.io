 ...
 * Time: 9:13:38 PM
 */
public class NewCountAggregation extends Aggregation {
    ...
    public void addValue(Value value) {
        ...
        if (value.type() != Value.EMPTY) {
            if (value.type() == Value.NUMBER && value.toDouble() != 0) {
                NumericValue numericValue = (NumericValue) value;
                Aggregation aggregation = numericValue.getAggregation();
                if (aggregation == null) {
                    addValue = 1;
                } else if (aggregation instanceof NewCountAggregation) {
                    NewCountAggregation countAggregation = (NewCountAggregation) aggregation;
