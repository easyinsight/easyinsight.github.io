 ...
 * Time: 12:32:36 PM
 */
public class HigherLevelComparator implements Comparator<MultiSummaryData.HigherLevel> {

    private List<SortKey> sortKeys;

    public HigherLevelComparator(AnalysisItem field, boolean ascending) {
        sortKeys = new ArrayList<>();
        sortKeys.add(new SortKey(field.createAggregateKey(), ascending ? 1 : -1));
    }

    public HigherLevelComparator() {
        sortKeys = new ArrayList<>();
    }
    ...
    public int compare(MultiSummaryData.HigherLevel row1, MultiSummaryData.HigherLevel row2) {
        int comparison = 0;
        for (SortKey sortKey : sortKeys) {
            Value value1 = row1.getRow().getValue(sortKey.key).toSortValue();
            Value value2 = row2.getRow().getValue(sortKey.key).toSortValue();
