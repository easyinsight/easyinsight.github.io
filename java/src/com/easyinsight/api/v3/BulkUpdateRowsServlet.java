...
public class BulkUpdateRowsServlet extends APIServlet {
    ...
    protected ResponseInfo processXML(Document document, EIConnection conn, HttpServletRequest request) throws Exception {
        ...
        try {
            Element updateNodes = (Element) document.query("/updates").get(0);
            Attribute dataSourceAttribute = updateNodes.getAttribute("dataSourceName");
            ...
            Nodes baseNodes = updateNodes.query("update");
            for (int updateCtr = 0; updateCtr < baseNodes.size(); updateCtr++) {


                if (baseNodes.size() == 0) {
                    throw new ServiceRuntimeException("The root element of the request needs to be update.");
                }
                Node baseNode = baseNodes.get(updateCtr);
                ...
                Nodes rowNodes = baseNode.query("rows/row");
                ...
                updates.add(new Update(dataSet, wheres));
            }
            dataStorage = DataStorage.writeConnection(dataSource, conn, SecurityUtil.getAccountID());
            for (Update update : updates) {
                dataStorage.updateData(update.dataSet, update.wheres);
            }
    ...
    private class Update {
        ...
        private List<IWhere> wheres;

        private Update(DataSet dataSet, List<IWhere> wheres) {
