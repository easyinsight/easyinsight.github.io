        ...
        try {
            ...
            for (int i = 0; i < caseNodes.size(); i++) {
                ...
                do {
                    Document notes;
                    if (notesOffset == 0) {
                        notes = runRestRequest("/kases/" + id + "/notes.xml", client, builder, highRiseCompositeSource.getUrl(), false, false);
                    } else {
                        notes = runRestRequest("/kases/" + id + "/notes.xml?n=" + notesOffset, client, builder, highRiseCompositeSource.getUrl(), false, false);
                    }
