    ...
    public DataSet getDataSet(Map<String, Key> keys, Date now, FeedDefinition parentDefinition, DataStorage dataStorage, EIConnection conn, String callDataID) {
        ...
        try {
            ...
            do {
                ...
                loadingProgress(0, 1, "Synchronizing with company notes...", callDataID);
                for (int i = 0; i < companyNodes.size(); i++) {
                    ...
                    do {
                        Document notes;
                        if (notesOffset == 0) {
                            notes = runRestRequest("/companies/" + id + "/notes.xml", client, builder, highRiseCompositeSource.getUrl(), false, false, parentDefinition);
                        } else {
                            notes = runRestRequest("/companies/" + id + "/notes.xml?n=" + notesOffset, client, builder, highRiseCompositeSource.getUrl(), false, false, parentDefinition);
                        }
