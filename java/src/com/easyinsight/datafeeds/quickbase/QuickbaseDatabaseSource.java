...
public class QuickbaseDatabaseSource extends ServerDataSourceDefinition {
    ...
    private static final String REQUEST = "<qdbapi><ticket>{0}</ticket><apptoken>{1}</apptoken><clist>{2}</clist><query>{3}</query><fmt>structured</fmt><options>num-1000</options></qdbapi>";
    private static final String REQUEST_2 = "<qdbapi><ticket>{0}</ticket><apptoken>{1}</apptoken><clist>{2}</clist><query>{3}<fmt>structured</fmt><options>num-1000.skp-{4}</options></qdbapi>";
    ...
    public DataSet getDataSet(Map<String, Key> keys, Date now, FeedDefinition parentDefinition, IDataStorage IDataStorage, EIConnection conn, String callDataID, Date lastRefreshDate) throws ReportException {
        ...
        try {
            return normalRetrieval(IDataStorage, quickbaseCompositeSource, sessionTicket, applicationToken, fullPath, lastRefreshDate);
            ...
            if (quickbaseCompositeSource.isPreserveCredentials()) {
                ...
                return normalRetrieval(IDataStorage, quickbaseCompositeSource, quickbaseCompositeSource.getSessionTicket(), applicationToken, fullPath, lastRefreshDate);
            } else {
                throw e;
            }
        }
    ...
    protected String getUpdateKeyName() {
        if (updateKeyName == null) {
            for (AnalysisItem field : getFields()) {
                String keyString = field.getKey().toBaseKey().toKeyString();
                if (keyString.endsWith(".3")) {
                    updateKeyName = keyString;
        ...
        throw new RuntimeException("Could not find a record ID field for " + getFeedName() + ".");
    }
    ...
    private DataSet normalRetrieval(IDataStorage IDataStorage, QuickbaseCompositeSource quickbaseCompositeSource, String sessionTicket,
                                    String applicationToken, String fullPath, Date lastRefreshDate) {
        HttpPost httpRequest = new HttpPost(fullPath);
        httpRequest.setHeader("Accept", "application/xml");
        httpRequest.setHeader("Content-Type", "application/xml");
        httpRequest.setHeader("QUICKBASE-ACTION", "API_DoQuery");
        ...
        try {
            do {
                ...
                String requestBody;
                if (masterCount == 0) {
                    requestBody = MessageFormat.format(REQUEST, sessionTicket, applicationToken, columnBuilder.toString(), query);
                } else {
                    requestBody = MessageFormat.format(REQUEST_2, sessionTicket, applicationToken, columnBuilder.toString(), query, String.valueOf(masterCount));
                }
                System.out.println(requestBody);
                //System.out.println(requestBody);
                byte[] contentBytes = requestBody.getBytes();
            ...
            if (lastRefreshDate != null) {
                ...
                Key recordIDKey = quickbaseCompositeSource.getField(getUpdateKeyName()).toBaseKey();

                for (IRow row : dataSet.getRows()) {
                    StringWhere userWhere = new StringWhere(recordIDKey, row.getValue(recordIDKey).toString());
