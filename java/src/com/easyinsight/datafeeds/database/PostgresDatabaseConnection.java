...
public class PostgresDatabaseConnection extends ServerDatabaseConnection {
    ...
    protected Connection createConnection() throws SQLException {
        String url = MessageFormat.format("jdbc:postgresql://{0}:{1}/{2}", host, String.valueOf(port), databaseName);
        return DriverManager.getConnection(url, dbUserName, dbPassword);
    }

    @Override
    public FeedType getFeedType() {
        return FeedType.SERVER_POSTGRES;
    }

    @Override
    public void beforeSave(EIConnection conn) throws Exception {
        super.beforeSave(conn);
        PreparedStatement queryStmt = conn.prepareStatement("SELECT postgres_database_connection.query_string FROM postgres_database_connection WHERE DATA_SOURCE_ID = ?");
    ...
    public void customStorage(Connection conn) throws SQLException {
        ...
        PreparedStatement deleteStmt = conn.prepareStatement("DELETE FROM postgres_database_connection WHERE DATA_SOURCE_ID = ?");
        deleteStmt.setLong(1, getDataFeedID());
        deleteStmt.executeUpdate();
        deleteStmt.close();
        PreparedStatement insertStmt = conn.prepareStatement("INSERT INTO postgres_database_connection (data_source_id, host_name, server_port, database_name," +
    ...
    public void customLoad(Connection conn) throws SQLException {
        ...
        PreparedStatement loadStmt = conn.prepareStatement("SELECT host_name, server_port, database_name, database_username," +
                "database_password, query_string, rebuild_fields, timeout FROM postgres_database_connection WHERE data_source_id = ?");
