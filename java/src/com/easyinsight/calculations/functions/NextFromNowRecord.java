 ...
 * Time: 7:58 PM
 */
public class NextFromNowRecord extends Function {
    ...
    public Value evaluate() {
        ...
        for (int i = 0; i < rows.size(); i++) {
            IRow row = rows.get(i);
            Value sortDate = row.getValue(sortField);
            if (sortDate.type() == Value.DATE) {
                DateValue dateValue = (DateValue) sortDate;
                Date date = dateValue.getDate();
                //System.out.println("comparing " + date + " with " + new Date());
                if (date.after(new Date())) {
                    System.out.println("\tafter!");
                    System.out.println("\treturning" + row.getValue(targetField));
