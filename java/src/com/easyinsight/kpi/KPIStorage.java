...
public class KPIStorage {
    ...
    private List<KPIUser> getKPIUsers(long kpiID, EIConnection conn) throws SQLException {
        PreparedStatement queryStmt = conn.prepareStatement("SELECT KPI_ROLE.USER_ID, GROUP_ID, USER.USERNAME, USER.EMAIL, USER.NAME, " +
                "COMMUNITY_GROUP.name, OWNER, RESPONSIBLE, USER.ACCOUNT_ID, USER.FIRST_NAME FROM " +
