...
public class TestSecurityProvider implements ISecurityProvider {
    ...
    public void setUserPrincipal(long userID) {
        ...
        userPrincipal = new UserPrincipal(user.getUserName(), user.getAccount().getAccountID(), user.getUserID(), user.getAccount().getAccountType(), user.isAccountAdmin(), user.isGuestUser());
    }
