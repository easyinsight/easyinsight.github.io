...
public class InternalDBServlet extends HttpServlet {
    ...
    private void newDatabase(Connection conn) throws SQLException {
        ...
        statement.execute("CREATE TABLE SQL_SERVER_CONFIG (HOST VARCHAR(100), PORT VARCHAR(10), DATABASE VARCHAR(100), " +
                "USERNAME VARCHAR(100), PASSWORD VARCHAR(100))");
        statement.execute("CREATE TABLE ORACLE_CONFIG (HOST VARCHAR(100), PORT VARCHAR(10), SID VARCHAR(100), " +
                "USERNAME VARCHAR(100), PASSWORD VARCHAR(100))");
