...
public class GraphDataProvider implements IDataProvider {
	...
	public function GraphDataProvider(host: SpringGraph): void {
		this.host = host;
	}
    ...
    private function onNewItem(event:NewItemEvent):void {
        nodes.push(makeGraphNode(event.item));
    }

    private function onNewLink(event:NewLinkEvent):void {
        edges.push(new GraphEdge(GraphNode(nodeStore[event.edge1ID]), GraphNode(nodeStore[event.edge2ID]), _distance));
        host.invalidateDisplayList();
    }

	public function set graph(g: Graph): void {
        if (_graph != null) {
            _graph.removeEventListener(NewItemEvent.NEW_ITEM, onNewItem);
            _graph.removeEventListener(NewLinkEvent.NEW_LINK, onNewLink);
        }
        this._graph = g;
        ...
        _graph.addEventListener(NewItemEvent.NEW_ITEM, onNewItem);
        _graph.addEventListener(NewLinkEvent.NEW_LINK, onNewLink);
	}
