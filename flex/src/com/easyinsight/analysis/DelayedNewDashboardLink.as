...
public class DelayedNewDashboardLink extends EventDispatcher
	{
		private var feedID:String;
		private var feedService:RemoteObject;
		
		public function DelayedNewDashboardLink(feedID:String)
		{
			this.feedID = feedID;
		...
		private function gotFeed(event:ResultEvent):void {
        	...
        	if (feedResponse.status == FeedResponse.SUCCESS) {
                dispatchEvent(new AnalyzeEvent(new PerspectiveInfo(PerspectiveInfo.DASHBOARD_EDITOR, { startDataSourceID:feedResponse.feedDescriptor.id})));
            } else {
