...
public class AutoCompleteManager extends EventDispatcher
{
       ...
       if(isTarget(textInput))
        {
	        if (!hasDropdown(textInput) )
	        {
                ...
                dropdown.addEventListener(FlexEvent.CREATION_COMPLETE, onStageAdd);
    ...
    private function onStageAdd(event:Event):void {
        //event.currentTarget.addEventListener(FlexMouseEvent.MOUSE_DOWN_OUTSIDE, dropdown_mouseDownOutsideHandler);
    }
			...
			if(_currentCollection !=null)
			{
	            ...
	            else
	            {
	            	...
	            	if(popUpEnabled)
	            	{
                        ...
                        //Alert.show(String(dropdown.x) + " - " + String(dropdown.y) + " - " + String(dropdown.width) + " - " + String(dropdown.height));
	...
	protected function updateDataProvider(textInput:TextInput):void
	{
        ...
        var txt:String = _currentTypedText;
        if (txt.length > 0 && txt.charAt(txt.length - 1) != "*") {
            txt += "*";
        }
        _pattern = new RegExp(createWildcardPattern(txt.toLowerCase()));
    ...
    private function createWildcardPattern(wildcard:String):String {
        var s:String = "";
        s += '^';
