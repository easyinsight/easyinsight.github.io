        ...
        case 'bottom':
          ...
          if (rotateLabels%360) {
            ...
            xTicks
              ...
              xLabelMargin = axisLabelDistance;
          }
          axisLabel.enter().append('text').attr('class', 'nv-axislabel');
          var w = (scale.range().length==2) ? scale.range()[1] : (scale.range()[scale.range().length-1]+(scale.range()[1]-scale.range()[0]));
          axisLabel
          ...
          */
          ...
          axisLabel.enter().append('text').attr('class', 'nv-axislabel');
              ...
              if (rotateYLabel && horizontalBar) {
                  var debugRange = scale.range();
                  var debugRangeLen = debugRange.length;

                  debugX = rotateYLabel ? (-scale.range()[debugRangeLen - 1] / 2) : -axis.tickPadding();
              } else {
...
chart.horizontalBar = function(_) {
    if (!arguments.length) return horizontalBar;
    horizontalBar = _;
    return chart;
}
      ...
      if (stacked)
        ...
        if (stacked && showValues) {
            bars.select('text')
                .attr('text-anchor', 'middle')
                .attr('y', function(d,i) {
                    var availableHeight = Math.max(Math.abs(y(d.y + (stacked ? d.y0 : 0)) - y((stacked ? d.y0 : 0))),1);
                    return y((stacked ? d.y1 : 0)) + (availableHeight / 2);
                    //return y((stacked ? d.y1 : 0));
                })
                .classed('stackLabel', true)
                .attr('dy', '.32em')
                .attr('width', x.rangeBand() / data.length)
                .text(function(d,i) {
                    var availableHeight = Math.max(Math.abs(y(d.y + (stacked ? d.y0 : 0)) - y((stacked ? d.y0 : 0))),1);
            ...
            ).attr('transform', function(d,i) { return 'translate(' + (x(getX(d,i)) + (x.rangeBand() / 2)) + ',0)'; });
        } else {
      ...
      if (stacked)
        ...
        if (showValues && stacked) {
            ...
            /*bars.transition()
             .select('text')
             .attr('x', function (d, i) {
             //return y(d.y1) + ((Math.abs(y(getY(d,i) + d.y0) - y(d.y0))) / 2);
             return y(d.y0);
             })*/
        }
          ...
          pieLabels.select(".nv-label text")
                ...
                .text(function(d, i) {
                  ...
                  var labelTypes = {
                    ...
                    "custom": valueFormat(getY(d.data)) + " / " + d3.format('%')(percent),
